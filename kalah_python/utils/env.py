...
class KalahEnv:
    ...
    def order_agent(self, agent: Agent):
        ...
        # have to commit action before execute action,
        # as it swaps the sides for me.
        agent.commit_action()
        env_state = self.execute_action(board=self.board,
                                        action=agent.action,
                                        side=agent.side)

        self.env_state = env_state
        # make_move was successful.
        # unregister the action when it's done
    ...
    def execute_action(self, board: Board, action: Action, side: Side) -> KalahEnvState:
        """
        ...
        if action == Action.SWAP:
            # the sides should not have been changed..
            assert side == Side.SOUTH and self.agent_n.side == Side.SOUTH
            return KalahEnvState.NORTH_TURN
        ...
        if finished_side:
            ...
            # game_ends
            return KalahEnvState.GAME_ENDS

        # your store minus opponent's store at the move
        if side == Side.SOUTH:
            return KalahEnvState.NORTH_TURN
        else:
            return KalahEnvState.SOUTH_TURN
...
class KalahACEnv:
    ...
    def reward_winner(self, winner: ACAgent, win_score: int):
        for idx, reward in enumerate(winner.reward_buffer):
            winner.reward_buffer[idx] = reward * (1 + self.h_params.BONUS_W) + win_score * self.h_params.BONUS_W

    def penalise_loser(self, loser: ACAgent, win_score: int):
        for idx, reward in enumerate(loser.reward_buffer):
            loser.reward_buffer[idx] = reward * (1 - self.h_params.BONUS_W) + win_score * (1 - self.h_params.BONUS_W)
